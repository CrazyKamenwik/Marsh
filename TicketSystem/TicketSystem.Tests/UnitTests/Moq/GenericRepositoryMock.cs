...
internal class GenericRepositoryMock<T> where T : IBaseEntity
{
    public static Mock<IGenericRepository<T>> GetMock(IEnumerable<T> baseEntities)
    {
        ...
        genericRepositoryMock.Setup(x => x.Create(baseEntities.First(), CancellationToken.None))
            .Returns(Task.CompletedTask);
        genericRepositoryMock.Setup(x => x.Update(CancellationToken.None, baseEntities.First()))
            .Returns(Task.CompletedTask);
        genericRepositoryMock.Setup(x => x.Remove(It.IsAny<int>(), CancellationToken.None))
            .Returns(Task.CompletedTask);
        genericRepositoryMock.Setup(x => x.GetByIdWithInclude(It.IsAny<int>(), CancellationToken.None,
                It.IsAny<Expression<Func<T, object>>[]>()))
            .ReturnsAsync(baseEntities.First());
        genericRepositoryMock.Setup(x => x.GetWithInclude(CancellationToken.None,
