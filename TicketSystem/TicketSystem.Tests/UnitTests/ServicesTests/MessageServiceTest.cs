...
namespace TicketSystem.Tests.UnitTests.ServicesTests
{
    public class MessageServiceTest
    {
        ...
        public MessageServiceTest()
        {
            ...
            _messageService = new MessageService(_userServiceMock.Object, _mapperMock.Object, _messageStrategiesMock.Object);
        }

        [Fact]
        public async Task Add_ShouldReturnMessageAndCallCorrectMessageStrategy_WhenUserRoleIsUser()
        {
            // Arrange
            ...
            _userServiceMock.Setup(x => x.GetUserById(It.IsAny<int>(), CancellationToken.None))
                .ReturnsAsync(new User { Id = 1, Name = "Vlad", UserRole = new UserRole() {Id = 1, Name = RolesConstants.User}});
            ...
            _userMessageStrategyMock.Verify(x => x.AddMessage(It.IsAny<Message>(), It.IsAny<User>(), CancellationToken.None));
        }

        [Fact]
        public async Task Add_ShouldReturnArgumentExceptionAndMessageNotEmpty_WhenUserRoleIsOperatorAndMessageHaveNoTicketId()
        {
            // Arrange
            var message = new Message() { Text = "Yup" };

            _userServiceMock.Setup(x => x.GetUserById(It.IsAny<int>(), CancellationToken.None))
                .ReturnsAsync(new User { Id = 1, Name = "Vlad", UserRole = new UserRole() { Id = 1, Name = RolesConstants.Operator } });

            // Act
            var result = await Assert.ThrowsAsync<ArgumentException>( () => _messageService.AddMessage(message, CancellationToken.None));
